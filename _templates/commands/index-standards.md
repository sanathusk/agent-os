...
Rebuild and maintain the standards index file (`index.toml`).
...
1. List all `.md` files in `better-agents/standards/` and its subfolders
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `better-agents/standards/` (not in a subfolder). Do not create an actual folder named "root".

### Step 2: Load Existing Index

Read `better-agents/standards/index.toml` if it exists. Note which entries already have descriptions.
...
2. Use {{ask_user_questions_tool}} to propose a description:
...
Generate `better-agents/standards/index.toml` with this structure:
...
**Note:** `root` appears first and contains standards files that live directly in `better-agents/standards/` (not in subfolders).
...
Updates `better-agents/standards/index.toml`
